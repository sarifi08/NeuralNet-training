...
def my_backward(x, y_target, w, cache):
    ...
    # ── Output layer ────────────────────────────────────────────────────────
    # MSE loss: d/dy = 2*(y - target) / (N * n_outputs)
    dy  = 2.0 * (y - y_target) / (n * y.shape[1])
    # tanh derivative: d(tanh(z))/dz = 1 - tanh(z)^2 = 1 - y^2
    dz3 = dy * (1.0 - y * y)
    dW3 = cache["a2"].T @ dz3      # (H2, N_OUT)
    db3 = dz3.sum(axis=0)          # (N_OUT,)

    # ── Hidden layer 2 ──────────────────────────────────────────────────────
    da2 = dz3 @ w["W3"].T          # (N, H2)
    dz2 = da2 * (cache["z2"] > 0) # ReLU mask
    dW2 = cache["a1"].T @ dz2     # (H1, H2)
...
def gradient_check():
    ...
    # Cast weights and inputs to float64 for the gradient check only.
    # float32 finite-differences lose too many significant digits when
    # (loss_p - loss_m) is computed — both losses round to the same value.
    # Real training stays in float32 throughout; this is check-only.
    w64 = {k: v.astype(np.float64) for k, v in nn_mod.init_weights(seed=0).items()}
    x = rng.normal(size=(8, N_FEATURES))            # float64
    y = rng.uniform(-1, 1, size=(8, N_ACTIONS))     # float64
